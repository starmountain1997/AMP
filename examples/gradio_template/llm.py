...
if __name__ == "__main__":
    with gr.Blocks() as demo:
        ...
        # Add state to track whether we're in the middle of generating a response
        is_generating = gr.State(False)

        gr.Markdown("# QwQ-32B 对话 (带可折叠推理过程)")
        gr.Markdown(
            "QwQ 是 Qwen 系列中的推理模型... (模型描述)"
        )

        # --- UI Setup ---
        # No separate reasoning display needed
        chatbot = gr.Chatbot(label="聊天记录", height=600, show_copy_button=True, sanitize_html=False, bubble_full_width=False) # sanitize_html=False is important!
        with gr.Row():
        ...
        with gr.Row():
            ...
            max_tokens = gr.Slider(minimum=100, maximum=8192, value=2048, step=100, label="Max Tokens")
        # --- End of UI Setup ---
        ...
        def user(user_message, history, session_id):
            ...
            # Clear input textbox
            return "", history + [[user_message, None]], session_id, history + [[user_message, None]]
        ...
        def bot(history, temperature_value, top_p_value, max_tokens_value, session_id):
            ...
            # Use the chat generator which yields (response_chunk, reasoning_chunk)
            for response_chunk, reasoning_chunk in chat(
                ...
                [h[:2] for h in history[:-1]], # Send previous turns' raw messages if stored differently
                ...
                if reasoning_chunk:  # Even if it's just starting to generate
                    ...
                    # Use <pre> for better formatting of potentially code-like reasoning
                    ...
                    # Once response is complete, next interaction will collapse it again
...
"""
                    ...
                    combined_message = f"{collapsible_reasoning_html}\n\n{full_final_response}"
                # --- End of Formatting ---

                # Update the assistant's message in the *last* turn of the history state
                history[-1][1] = combined_message

                # Yield updates: history state, session_id state, chatbot display
                yield history, session_id, history
            
            # When generation is complete, update to close the details by default for future displays
...
"""
            ...
            logger.info(f"Session {session_id} - Final Reasoning (raw): {full_reasoning}")
        # --- End of Bot Function ---


        # --- Event Handling ---
        # Define outputs - only history state, session state, and chatbot component
