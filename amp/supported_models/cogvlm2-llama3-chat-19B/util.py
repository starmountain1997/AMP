...
def apply_rotary(
    ...
    for i in range(batch):
        if isinstance(seqlen_offsets, torch.Tensor):
            start_idx = seqlen_offsets[i]
            seq_len = cu_seqlens[i + 1] - start_idx if cu_seqlens is not None else seqlen
            x_batch = x[i, start_idx:start_idx + seq_len]
            cos_batch = cos[i, :seq_len]
            sin_batch = sin[i, :seq_len]
        else:
            x_batch = x[i]
            cos_batch = cos
            sin_batch = sin

        cos_batch = cos_batch.unsqueeze(-1)  # Add headdim dimension
        sin_batch = sin_batch.unsqueeze(-1)  # Add headdim dimension

        # Apply cosine and sine rotations
        x_rot = x_batch * cos_batch - x_batch.flip(-1) * sin_batch
        x_rot = x_rot + x_batch * sin_batch + x_batch.flip(-1) * cos_batch
        
        output[i, :seq_len] = x_rot
