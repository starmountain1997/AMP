...
def operator_rank(csv_path: str):
    ...
    grouped_sum = (
        df.groupby(["Name"])[["Device Total Duration(us)", "Host Total Duration(us)"]]
        .sum()
        .reset_index()
    )
    grouped_sum["Total Duration"] = (
        grouped_sum["Device Total Duration(us)"]
        + grouped_sum["Host Total Duration(us)"]
    )
